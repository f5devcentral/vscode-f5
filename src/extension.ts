...
export async function activate(context: ExtensionContext) {
	...
	const oldExt = extensions.all.filter( el => el.id === oldExtName );
	const newExt = extensions.all.filter( el => el.id === newExtName );
	...
	if (oldExt && newExt) {
		...
		console.log('both extensions detected:', oldExt, newExt);

		await window.showWarningMessage('Please uninstall the old vscode-f5-fast extension!');
		...
		// 		const reld = await window.showInformationMessage('It should be uninstalled now, reload?', {modal: true}, 'ok');
		// 		if (reld) {
