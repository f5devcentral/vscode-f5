...
suite('Extension Test Suite', () => {
	...
	test('open JSON editor -> insert as3 snippet', async () => {
		...
		await vscode.commands.executeCommand('editor.action.insertSnippet', { name: "example_F5_AS3_declaration"});

		//	check snippet against isolated copy
		assert.deepStrictEqual(as3_sample_01, jsonEditor.getText());
		
		// utils.displayJsonInEditor({ "key1": "value1", "key2": "value2"});
		...
		// 	// const jsonEditor = await vscode.workspace.openTextDocument({ language: 'json', content: as3_sample_01 });
		...
		// 	// await new Promise(resolve => { setTimeout(resolve, 2000); });
		// 	// await vscode.commands.executeCommand('editor.action.insertSnippet', {snippet: 'as3-Sample_01'} );
		// 	await vscode.commands.executeCommand('editor.action.insertSnippet', { name: "example_F5_AS3_declaration"});
			
		// 	// progress.report({ message: `${response.body.message}`});
			
		// 	// const editorText = JSON.parse(jsonEditor.getText());
		// 	// const editorText = editor.document.getText();

		// 	// const exptd = as3_sample_01b;
		// 	// const exptd =  

		// 	// debugger;
		// 	assert.deepStrictEqual(as3_sample_01, jsonEditor.getText());

		// 	// assert.equal(JSON.stringify(as3_sample_01b), JSON.stringify(editorText));
		// 	// assert.equal(exptd, editorText);
		// 	// assert.equal(1, 1);
		// });
	...
	test('open test json in editor #1', async () => {
		await vscode.commands.executeCommand('workbench.action.closeAllEditors');
	...
	test('open test json in editor #2', async () => {
		await vscode.commands.executeCommand('workbench.action.closeAllEditors');
